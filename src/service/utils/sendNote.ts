...
const emailMap: { [key: string]: any } = {
  [UserAuthTypeEnum.register]: {
    subject: '注册 智能知识库(beta) 账号',
    html: (code: string) => `<div>智能知识库(beta) 账号，验证码为：${code}</div>`
  },
  [UserAuthTypeEnum.findPassword]: {
    subject: '修改 智能知识库(beta) 密码',
    html: (code: string) => `<div>您正在修改 智能知识库(beta) 账号密码，验证码为：${code}</div>`
  }
};
